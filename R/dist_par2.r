...
#   return(c(x=x.col1,y=x.col2))
  if(share_w == 1){
	...
	stopCluster(cl)
	if (distance == "paths"){
	  return(c(Spp=dpr,Spep=dl,Spelp=dlpr,Spinp=dnpr, direct = 1.0*x.col1[2]/(x.col1[1]+x.col1[2]), reverse = 1.0*x.col2[2]/(x.col2[1]+x.col2[2])))
	} else if(distance == "transfer"){
	  return(c(direct = 1.0*x.col1[2]/(x.col1[1]+x.col1[2]), reverse = 1.0*x.col2[2]/(x.col2[1]+x.col2[2])))}
